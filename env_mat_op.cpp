...
// 编译: clang++ -O3 -march=armv9-a+sve2 -o benchmark_practical
// env_mat_a_practical_optimized.cpp
...
inline void spline5_switch_scalar(float& sw, float& dsw, float r, float rmin,
                                  float rmax) {
    if (r < rmin) {
        sw = 1.0f;
        dsw = 0.0f;
...
__arm_locally_streaming inline void
spline5_switch_sve(svfloat32_t& vec_sw, svfloat32_t& vec_dsw, svfloat32_t vec_r,
                   float rmin, float rmax, svbool_t pg) {
    ...
    if (svptest_any(pg, mask_middle)) {
        ...
        svfloat32_t vec_uu =
            svmul_x(mask_middle, svsub_x(mask_middle, vec_r, vec_rmin), vec_du);
        ...
        // sw = uu³(-6uu² + 15uu - 10) + 1
        svfloat32_t poly =
            svmad_x(mask_middle, vec_uu, svdup_f32(15.0f), svdup_f32(-10.0f));
        ...
        svfloat32_t term1 = svmul_x(
            mask_middle, svmul_x(mask_middle, svdup_f32(3.0f), uu2), poly);
        svfloat32_t term2 =
            svmad_x(mask_middle, vec_uu, svdup_f32(-12.0f), svdup_f32(15.0f));
        term2 = svmul_x(mask_middle, uu3, term2);
        svfloat32_t dsw_mid =
            svmul_x(mask_middle, svadd_x(mask_middle, term1, term2), vec_du);
...
void env_mat_a_cpu_baseline(std::vector<float>& descrpt_a,
                            std::vector<float>& descrpt_a_deriv,
                            std::vector<float>& rij_a,
                            const std::vector<float>& posi,
                            const std::vector<int>& type, const int& i_idx,
                            const std::vector<int>& fmt_nlist_a,
                            const std::vector<int>& sec_a, const float& rmin,
                            const float& rmax) {
    ...
    for (int sec_iter = 0; sec_iter < int(sec_a.size()) - 1; ++sec_iter) {
        for (int nei_iter = sec_a[sec_iter]; nei_iter < sec_a[sec_iter + 1];
            ...
            // 12个导数分量
            descrpt_a_deriv[nei_iter * 12 + 0] =
                rx * inr3 * sw - desc0 * dsw * rx * inr;
            descrpt_a_deriv[nei_iter * 12 + 1] =
                ry * inr3 * sw - desc0 * dsw * ry * inr;
            descrpt_a_deriv[nei_iter * 12 + 2] =
                rz * inr3 * sw - desc0 * dsw * rz * inr;

            descrpt_a_deriv[nei_iter * 12 + 3] =
                (2.0f * rx * rx * inr4 - inr2) * sw - desc1 * dsw * rx * inr;
            descrpt_a_deriv[nei_iter * 12 + 4] =
                (2.0f * rx * ry * inr4) * sw - desc1 * dsw * ry * inr;
            descrpt_a_deriv[nei_iter * 12 + 5] =
                (2.0f * rx * rz * inr4) * sw - desc1 * dsw * rz * inr;

            descrpt_a_deriv[nei_iter * 12 + 6] =
                (2.0f * ry * rx * inr4) * sw - desc2 * dsw * rx * inr;
            descrpt_a_deriv[nei_iter * 12 + 7] =
                (2.0f * ry * ry * inr4 - inr2) * sw - desc2 * dsw * ry * inr;
            descrpt_a_deriv[nei_iter * 12 + 8] =
                (2.0f * ry * rz * inr4) * sw - desc2 * dsw * rz * inr;

            descrpt_a_deriv[nei_iter * 12 + 9] =
                (2.0f * rz * rx * inr4) * sw - desc3 * dsw * rx * inr;
            descrpt_a_deriv[nei_iter * 12 + 10] =
                (2.0f * rz * ry * inr4) * sw - desc3 * dsw * ry * inr;
            descrpt_a_deriv[nei_iter * 12 + 11] =
                (2.0f * rz * rz * inr4 - inr2) * sw - desc3 * dsw * rz * inr;
        }
    }
}
...
__arm_locally_streaming void env_mat_a_cpu_practical_optimized(
    std::vector<float>& descrpt_a, std::vector<float>& descrpt_a_deriv,
    std::vector<float>& rij_a, const std::vector<float>& posi,
    const std::vector<int>& type, const int& i_idx,
    const std::vector<int>& fmt_nlist_a, const std::vector<int>& sec_a,
    ...
    for (int sec_iter = 0; sec_iter < int(sec_a.size()) - 1; ++sec_iter) {
        ...
        for (int batch_start = sec_start; batch_start < sec_end;
            ...
            // 1. 收集一个批次的数据
            int batch_count = 0;
            for (int jj = batch_start;
                 jj < sec_end && batch_count < MAX_BATCH &&
                 jj < batch_start + SVL;
                 ++jj) {
                if (fmt_nlist_a[jj] < 0)
                    break;
            ...
            svfloat32_t correction = svmls_f32_m(pg, one_half, vec_nr2,
                                                 svmul_f32_x(pg, inr_sq, half));
            ...
            // 导数 0-2: ∂(1/r)/∂{x,y,z}
            svfloat32_t common_0 =
                svmul_f32_x(pg, svmul_f32_x(pg, vec_desc0, vec_dsw), vec_inr);
            svfloat32_t term1_0 = svmul_f32_x(pg, vec_inr3, vec_sw);

            svfloat32_t deriv_0 = svmls_f32_m(
                pg, svmul_f32_x(pg, vec_x, term1_0), common_0, vec_x);
            svfloat32_t deriv_1 = svmls_f32_m(
                pg, svmul_f32_x(pg, vec_y, term1_0), common_0, vec_y);
            svfloat32_t deriv_2 = svmls_f32_m(
                pg, svmul_f32_x(pg, vec_z, term1_0), common_0, vec_z);

            // 导数 3-5: ∂(x/r²)/∂{x,y,z}
            svfloat32_t common_1 =
                svmul_f32_x(pg, svmul_f32_x(pg, vec_desc1, vec_dsw), vec_inr);

            svfloat32_t term_xx = svsub_f32_x(
                pg,
                svmul_f32_x(
                    pg,
                    svmul_f32_x(pg, svmul_f32_x(pg, vec_x, vec_x), vec_inr4),
                    vec_two),
                vec_inr2);
            svfloat32_t deriv_3 = svmls_f32_m(
                pg, svmul_f32_x(pg, term_xx, vec_sw), common_1, vec_x);

            svfloat32_t term_xy = svmul_f32_x(
                pg, svmul_f32_x(pg, svmul_f32_x(pg, vec_x, vec_y), vec_inr4),
                vec_two);
            svfloat32_t deriv_4 = svmls_f32_m(
                pg, svmul_f32_x(pg, term_xy, vec_sw), common_1, vec_y);

            svfloat32_t term_xz = svmul_f32_x(
                pg, svmul_f32_x(pg, svmul_f32_x(pg, vec_x, vec_z), vec_inr4),
                vec_two);
            svfloat32_t deriv_5 = svmls_f32_m(
                pg, svmul_f32_x(pg, term_xz, vec_sw), common_1, vec_z);

            // 导数 6-8: ∂(y/r²)/∂{x,y,z}
            svfloat32_t common_2 =
                svmul_f32_x(pg, svmul_f32_x(pg, vec_desc2, vec_dsw), vec_inr);

            svfloat32_t term_yx = svmul_f32_x(
                pg, svmul_f32_x(pg, svmul_f32_x(pg, vec_y, vec_x), vec_inr4),
                vec_two);
            svfloat32_t deriv_6 = svmls_f32_m(
                pg, svmul_f32_x(pg, term_yx, vec_sw), common_2, vec_x);

            svfloat32_t term_yy = svsub_f32_x(
                pg,
                svmul_f32_x(
                    pg,
                    svmul_f32_x(pg, svmul_f32_x(pg, vec_y, vec_y), vec_inr4),
                    vec_two),
                vec_inr2);
            svfloat32_t deriv_7 = svmls_f32_m(
                pg, svmul_f32_x(pg, term_yy, vec_sw), common_2, vec_y);

            svfloat32_t term_yz = svmul_f32_x(
                pg, svmul_f32_x(pg, svmul_f32_x(pg, vec_y, vec_z), vec_inr4),
                vec_two);
            svfloat32_t deriv_8 = svmls_f32_m(
                pg, svmul_f32_x(pg, term_yz, vec_sw), common_2, vec_z);

            // 导数 9-11: ∂(z/r²)/∂{x,y,z}
            svfloat32_t common_3 =
                svmul_f32_x(pg, svmul_f32_x(pg, vec_desc3, vec_dsw), vec_inr);

            svfloat32_t term_zx = svmul_f32_x(
                pg, svmul_f32_x(pg, svmul_f32_x(pg, vec_z, vec_x), vec_inr4),
                vec_two);
            svfloat32_t deriv_9 = svmls_f32_m(
                pg, svmul_f32_x(pg, term_zx, vec_sw), common_3, vec_x);

            svfloat32_t term_zy = svmul_f32_x(
                pg, svmul_f32_x(pg, svmul_f32_x(pg, vec_z, vec_y), vec_inr4),
                vec_two);
            svfloat32_t deriv_10 = svmls_f32_m(
                pg, svmul_f32_x(pg, term_zy, vec_sw), common_3, vec_y);

            svfloat32_t term_zz = svsub_f32_x(
                pg,
                svmul_f32_x(
                    pg,
                    svmul_f32_x(pg, svmul_f32_x(pg, vec_z, vec_z), vec_inr4),
                    vec_two),
                vec_inr2);
            svfloat32_t deriv_11 = svmls_f32_m(
                pg, svmul_f32_x(pg, term_zz, vec_sw), common_3, vec_z);
...
void generate_test_data(std::vector<float>& posi, std::vector<int>& type,
                        std::vector<int>& fmt_nlist_a, std::vector<int>& sec_a,
                        int& i_idx, int num_atoms, int avg_neighbors) {
    srand(42);
...
bool verify_results(const std::vector<float>& result1,
                    const std::vector<float>& result2, const char* name,
    ...
    printf("  %s %s: max_err=%.2e, errors=%d/%zu\n",
           diff_count == 0 ? "✅" : "⚠️", name, max_diff, diff_count,
           result1.size());
...
void run_benchmark(int num_atoms, int avg_neighbors, int iterations) {
    ...
    generate_test_data(posi, type, fmt_nlist_a, sec_a, i_idx, num_atoms,
                       avg_neighbors);
    ...
    for (int i = 0; i < iterations; i++) {
        env_mat_a_cpu_baseline(desc_base, deriv_base, rij_base, posi, type,
                               i_idx, fmt_nlist_a, sec_a, rmin, rmax);
    }
    ...
    for (int i = 0; i < iterations; i++) {
        env_mat_a_cpu_practical_optimized(desc_opt, deriv_opt, rij_opt, posi,
                                          type, i_idx, fmt_nlist_a, sec_a, rmin,
                                          rmax);
    }
